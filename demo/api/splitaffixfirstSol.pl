...
# usage:
# perl -w splitaffix.pl < infile.txt > stemmedAffix.txt
# were "infile" is the input text in Arabic Windows encoding
# and "stemmedAffix" is the output stemmed text
################################################################################
#
                        ...
                        foreach $suffix_value (@{$suffix_hash{$suffix}}) {
                           ...
                           if ( exists($hash_AC{"$cat_a"." "."$cat_c"}) ) {
                              if ( exists($hash_BC{"$cat_b"." "."$cat_c"}) ) {
                                 ...
                                 $cnt++; push (@solutions, "$Aprefix $Astem $Asuffix "); #   SOLUTION $cnt: ($voc_a$voc_b$voc_c) [$lemmaID] $pos_a$pos_b$pos_c\n     (GLOSS): $gloss_a + $gloss_b + $gloss_c\n");
                                 last OUTER;
                              }
                           }
                        }
